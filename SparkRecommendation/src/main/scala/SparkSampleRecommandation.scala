...
object SparkSampleRecommandation {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //    val moviesDf = sparkSession.read.text(dataPath + "movies.dat")
    //      .toDF("value").withColumn("value", split(col("value"), "::")).select(
    //      col("value").getItem(0).as("movieId"),
    //      col("value").getItem(1).as("movieName")
    //    )

    val movies = sparkSession.sparkContext.textFile(dataPath + "movies.dat").map {
      line =>
    ...
    val ratings = sparkSession.sparkContext.textFile(dataPath + "ratings.dat").map {
      line =>
        val fields = line.split("::");
    ...
    val candidates = sparkSession.sparkContext.parallelize(movies.keys.toSeq)
